...
### How it's produced (important — affects the design)
The image is generated by rendering a **dedicated HTML "capture surface"** and converting
that DOM to an image with **html-to-canvas** (`html-to-image` / `html2canvas`, already in the
project). So the thing being screenshotted is a **real on-screen view that the designer must
design separately** — not the live dashboard itself.

- **Separate, on-click preview.** Clicking "Capture / Share" opens a **dedicated preview**
  (its own screen/modal) that shows **exactly** what the final image will look like —
  what-you-see-is-what-you-get. The user reviews it, then captures/downloads/shares from there.
- This preview surface **is** the capture target: design it as a self-contained "poster" with
  its own composition, independent of the surrounding app chrome and the responsive viewport.
  Give it a **fixed, predictable canvas size** (e.g. a portrait share format) so the export is
  consistent rather than depending on screen width.
- It must render reliably for html-to-canvas: avoid effects the converter can't capture; make
  sure fonts, avatars/logo, and colors are fully loaded/embedded before capture.

### Behavior & functionality
- Trigger → open the **preview** (live data composed onto the capture surface) → user reviews →
  **download** (PNG) and/or **share** (native share sheet on mobile where available).
- The preview should clearly show its **as-of date/time** and the captured content (club
  summary + members items), since that's what gets frozen into the image.
- **Scales with member count:** the capture surface must look good for both a handful and many
  members — the designer decides how (compact rows, multiple columns, or splitting into
  multiple images/pages when the list is long). Don't let it become an unreadable wall.
- **Crisp output:** render at high enough resolution/pixel-ratio to stay sharp when zoomed on
  a phone (html-to-canvas supports a scale/pixelRatio factor).
- Optional toggles the designer may offer in the preview: choose which figures to include,
  include/exclude inactive members, light/dark version of the image.
