...
export async function POST() {
  try {
    ...
    const members = await prisma.member.findMany({ include: { vendors: true, passbook: true, transactionFrom: true, transactionTo: true } });
    const vendors = await prisma.vendor.findMany({ include: { transactions: true, profitShares: true, passbook: true } });
