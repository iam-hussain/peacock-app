...
export function TransactionForm({
    ...
    const form = useForm<TransactionFormSchema>({
        ...
        defaultValues: selected
            ? {
                ...
                toId: selected.toId || "",
                transactionType: selected.type || selected.transactionType || "PERIODIC_DEPOSIT",
    ...
    const formToLabels = useMemo(() => {
        ...
        if (["FUNDS_TRANSFER"].includes(transactionType)) return ["Club - FROM", "Club - TO"]
        if (["VENDOR_RETURNS"].includes(transactionType)) return ["Vendor - FROM", "Club - TO"]
        if (["LOAN_REPAY", "LOAN_INTEREST"].includes(transactionType)) return ["Loan - FROM", "Club - TO"]
        if (transactionType === "VENDOR_INVEST") return ["Club - FROM", "Vendor - TO"]
    ...
    const mutation = useMutation({
        ...
        onSuccess: async ({ transaction }: any = {}) => {
            if (!selected) {
                form.reset({
                    ...transaction,
                    transactionAt: transaction?.occurredAt ? newZoneDate(transaction.occurredAt) : newZoneDate(),
            ...
            toast.success(selected ? "Transaction successfully updated!" : "Transaction successfully added!")
            onSuccess?.()
        },
        onError: (error: any) => {
        ...
        <Form {...form}>
            ...
            >
                        ...
                        <FormItem>
                            ...
                            <FormControl>
                                <Select onValueChange={field.onChange} defaultValue={field.value}>
                                    ...
                                    <SelectContent>
                                        {Object.entries(transactionTypeHumanMap).map(([key, name]) => (
                ...
                {isMobile ? (
                    <>
                        <FormField
                            ...
                            render={({ field }) => (
                                <FormItem>
                                    <FormLabel className="text-sm font-medium">{formToLabels[0]} *</FormLabel>
                                    <FormControl>
                                        <Select onValueChange={field.onChange} defaultValue={field.value}>
                                            <SelectTrigger className="h-11 rounded-lg">
                                                <SelectValue
                        ...
                        <FormField
                            ...
                            render={({ field }) => (
                                <FormItem>
                                    <FormLabel className="text-sm font-medium">{formToLabels[1]} *</FormLabel>
                                    <FormControl>
                                        <Select onValueChange={field.onChange} defaultValue={field.value}>
                                            <SelectTrigger className="h-11 rounded-lg">
                                                <SelectValue
                ...
                <FormField
                    ...
                    render={({ field }) => (
                        <FormItem>
                            <FormLabel className="text-sm font-medium">Note (Optional)</FormLabel>
                            <FormControl>
                                <Textarea
